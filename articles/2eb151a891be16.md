...
```typescript
class GetPokemonUnexpectedError extends ErrorFactory({
  name: 'GetPokemonUnexpectedError',
...
- ハンドリングが必要でないエラー（fetch()やjson()が失敗した場合のエラー）は`GetPokemonUnexpectedError`のResult型として返す

```typescript
type GetPokemonError = ValidationError | HttpError | GetPokemonUnexpectedError;
...
const getPokemon = async (pokemonName: string): Result.ResultAsync<Pokemon, GetPokemonError> => {
  try {
    ...
    return Result.fail(new GetPokemonUnexpectedError({ cause: error }));
  }
}
```
...
```typescript
// エラーが発生した場合
console.error(result.error); // GetPokemonUnexpectedError - アプリケーションコードを指すスタックトレース
...
```typescript
if (result.error instanceof GetPokemonUnexpectedError) {
